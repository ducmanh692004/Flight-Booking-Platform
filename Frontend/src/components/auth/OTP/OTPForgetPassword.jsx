...
const OtpForgetPassword = ({ email }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const emailStored = localStorage.getItem('emailForgetReset');
            const response = await handleConfirmOtpForgetPassword(
                emailStored || email,
                otp
            );

            if (response && response.EC === 0) {
                history.push('/confirm-new-password');
            } else {
                toast.error(response.EM || `${'otpForgetPassword.exprerOtp'}`);
            }
        } catch (err) {
            console.log(error);
        }
    ...
    const handleSendOtpAgain = async () => {
        try {
            const emailStored = localStorage.getItem('emailForgetReset');
            if (emailStored !== '') {
                const response = await handleSendEmailResetPassword(
                    emailStored
                );
                if (response && response.EC === 0) {
